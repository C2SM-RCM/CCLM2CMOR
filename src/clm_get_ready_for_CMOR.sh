...
# concatenate monthly files to annual file
  ...
  do
    ...
    then
      if ${overwrite}
      then
        echon "Directory ${FILEOUT} already exists. Overwriting..."
      else
        echon "Directory ${FILEOUT} already exists. Use option -o to overwrite. Skipping..."
        continue
      fi
    fi
...
# Mean wind spdeed at 10m height: SP_10M
  ...
  then
    ...
    then
      ...
      echo $(basename ${file3}) " already exists. Use option -o to overwrite. Skipping..."
    fi
  else
