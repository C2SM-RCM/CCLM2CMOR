...
#HJP _April 15th Begin  
# move the following part behind the definition of LOG-Files
...
#
#HJP March 2019 Begin
# definintion of LOG-files after the setting of varlist and extension of name of Log-File by element "0" (the first entry in varlist) of varlist,
...
#HJP _April 15th Begin  
# following part must appear after the definition of LOG-Files since messages might be written in these LOG-files; therefore, the files must exist 
    if not limit_range and int(options.multi) > 1:
        print("To use multiprocessing you have to limit the time range by specifying this range over the command line (-s START, -e END)! Exiting...")
        log.error("To use multiprocessing you have to limit the time range by specifying this range over the command line (-s START, -e END)! Exiting...")
        sys.exit()
